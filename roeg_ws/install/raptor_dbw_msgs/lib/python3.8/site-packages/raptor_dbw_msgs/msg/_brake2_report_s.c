...
bool raptor_dbw_msgs__msg__brake2_report__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // brake_pressure
    PyObject * field = PyObject_GetAttrString(_pymsg, "brake_pressure");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->brake_pressure = (float)PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
...
PyObject * raptor_dbw_msgs__msg__brake2_report__convert_to_py(void * raw_ros_message)
{
  ...
  {  // brake_pressure
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->brake_pressure);
    {
      int rc = PyObject_SetAttrString(_pymessage, "brake_pressure", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
