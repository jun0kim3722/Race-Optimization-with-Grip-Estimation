...
bool raptor_dbw_msgs__msg__steering2_report__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // expect_torque_driver
    PyObject * field = PyObject_GetAttrString(_pymsg, "expect_torque_driver");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->expect_torque_driver = (float)PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
...
PyObject * raptor_dbw_msgs__msg__steering2_report__convert_to_py(void * raw_ros_message)
{
  ...
  {  // expect_torque_driver
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->expect_torque_driver);
    {
      int rc = PyObject_SetAttrString(_pymessage, "expect_torque_driver", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
