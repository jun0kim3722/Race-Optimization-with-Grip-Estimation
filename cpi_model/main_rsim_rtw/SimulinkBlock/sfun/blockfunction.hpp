...
static inline int call_cgSystemReset(CodeConstructor* pCodeCtor) {
    auto* pBlock = static_cast<Block*>(pCodeCtor);
    return call_Block_method(pBlock, &Block::cgSystemReset);
}
...
SL_CORE_BLOCK_EXPORT_CLASS void SFun_set_cgSystemReset(SFun_BlockFunctionTable* pFcnTable,
                                                       SFun_BlockFunction pfn_call_cgSystemReset);
...
build_BlockFunctionTable(DerivedBlock* = NULL) {
    ...
    SFun_set_cgSystemReset(pFcnTable.get(), &call_cgSystemReset);
