...
slMessage* _slMsgSvcCreateMsgWithId(slMsgManager* msgMgr,
    ...
    if (data == NULL) {
        ...
        if (_slMsgIsDataPointerPool(msgWriterPayloadPool)) {
            ...
            __slmsg_assert(
                _slMsgCallDeepCopy(msgWriterPayloadPool, msg->fData, (const slMessage*)data));
        } else {
...
slMessage* _slMsgSvcSRSWSendMsg(slMsgManager* msgMgr, slMessage* msg, slMsgQueueId queueId) {
    ...
    if (SLMSG_CIRCULAR_INDEX(q->fSRSWFIFOQueue.fCircularTail + 1,
        ...
        if (_slMsgIsDataPointerPool(msgWriterPool)) {
            ...
            __slmsg_assert(_slMsgCallDeepCopy(msgWriterPool, dst, (const slMessage*)msg->fData));
        } else {
...
void _slMsgCopyDataFromMsg(slMsgManager* mgr, void* buf, slMessage* msg, size_t bufsize) {
    ...
    if (_slMsgIsDataPointerPool(pool)) {
        ...
        __slmsg_assert(_slMsgCallDeepCopy(pool, buf, (const slMessage*)msg->fData));
    } else {
