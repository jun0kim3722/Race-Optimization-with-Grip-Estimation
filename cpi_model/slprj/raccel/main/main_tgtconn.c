...
#endif
if ( writePortNumToServerInfoFile ) { FILE * fh ; fh = fopen ( serverInfoFileName , "w" ) ; if ( fh == NULL ) { result = "Unable to create target connectivity server info file\n" ; return ( result ) ; } ( void ) fprintf ( fh , "Server Port Number: %d\n" , ( unsigned int ) port ) ; fclose ( fh ) ; } } if ( startParamTuningTgtAppSvc ( ) ) { result = "Could not start ParamTuning app service" ; return ( result ) ; } return ( result ) ; } EXTERN_C void TgtConnTerm ( ) { terminateParamTuningTgtAppSvc ( ) ; } EXTERN_C void TgtConnPreStep ( int_T tid ) { UNUSED_PARAMETER ( tid ) ; tunePendingParameterChanges ( rt_modelMapInfoPtr ) ; } EXTERN_C void TgtConnPostStep ( int_T tid ) { UNUSED_PARAMETER ( tid ) ; }
