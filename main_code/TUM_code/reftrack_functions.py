...
def calc_trackboundaries(reftrack: np.ndarray, safety_margin: float=0.0) -> tuple:
    """
    Documentation
    ...
    track_boundary_right = reftrack[:, :2] + refline_normvecs[:, :2] * (np.expand_dims(reftrack[:, 2], axis=1) - safety_margin)
    track_boundary_left = reftrack[:, :2] - refline_normvecs[:, :2] * (np.expand_dims(reftrack[:, 3], axis=1) - safety_margin)
