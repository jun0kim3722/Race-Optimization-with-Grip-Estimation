...
class FrictionMapInterface:
    ...
    def update_friction_coeff(self, positions: np.ndarray, new_coeff: np.ndarray) -> None:
        """
        This function updates the friction value mue for a given position.
        ...
        # query requested positions to get indices of grid cells containing the corresponding mue values
        _, map_idxs = self.tpa_map.query(positions)

        # upate mue-value(s) from dictionary
        for i, map_idx in enumerate(map_idxs):
            self.tpa_data[map_idx] = new_coeff[i]
